...
window.addEventListener('appinstalled', () => {
  ...
  showToast(t('toast_install_success'), 'success');
});
function installApp() {
  if (!_deferredPrompt) {
    showToast(t('toast_share_tip'), 'success');
    return;
  }
...
function setHeaderDate() {
  const d = new Date();
  const localeMap = { 'en': 'en-IN', 'hi-hg': 'hi-IN', 'hi-dv': 'hi-IN' };
  document.getElementById('headerDate').textContent =
    d.toLocaleDateString(localeMap[currentLang] || 'hi-IN', { weekday: 'short', day: 'numeric', month: 'long' });
}
setHeaderDate();
...
function addWorker() {
  ...
  if (!name || !rate) { showToast(t('toast_name_rate_req'), 'error'); return; }
  ...
  renderWorkers();
  renderDashboard();
  showToast(t('toast_add_success', { name }));
}
...
function saveEditWorker() {
  ...
  renderWorkers();
  renderDashboard();
  showToast(t('toast_update_success'));
}
...
function deleteWorker() {
  ...
  confirmText.innerHTML = `<span style="font-weight:800; color:#fff;">${w.name}</span> ` + t('delete_confirm_sub', { name: '' });
...
function confirmDeleteWorker() {
  ...
  renderWorkers();
  renderDashboard();
  showToast(t('toast_worker_removed'), 'error');
}
...
function renderWorkers() {
  ...
  if (!DB.workers.length) {
    grid.innerHTML = `<div class="empty" style="grid-column:1/-1">
      ...
      <div class="empty-title" data-i18n="empty_no_workers">${t('empty_no_workers')}</div>
      <div class="empty-sub" data-i18n="empty_add_worker_tip">${t('empty_add_worker_tip')}</div>
    </div>`;
    return;
  }
  ...
  grid.innerHTML = DB.workers.map(w => {
    ...
    const attMap = { 
      present: { label: t('present'), cls: 'tag-green' }, 
      absent: { label: t('absent'), cls: 'tag-red' }, 
      half: { label: t('half_day'), cls: 'tag-yellow' }, 
      overtime: { label: t('overtime'), cls: 'tag-blue' }, 
      none: { label: '', cls: '' } 
    };
    ...
    return `<div class="worker-card" onclick="openEditWorker('${w.id}')">
      ...
      <div class="worker-skill">${t('skills.' + w.skill)} ${w.phone ? '· 📞 ' + w.phone : ''}</div>
      <div class="worker-rate">₹${w.rate.toLocaleString('en-IN')}<span>/${t('stat_today').toLowerCase().replace('aaj ka din', 'din')}</span></div>
      <div class="worker-meta">
        ${att.cls ? `<span class="worker-tag ${att.cls}">${att.label}</span>` : ''}
        ${totalAdv > 0 ? `<span class="worker-tag tag-red">${t('nav_advance')} ₹${totalAdv.toLocaleString('en-IN')}</span>` : ''}
        ${w.otRate ? `<span class="worker-tag tag-blue">${t('overtime')} ₹${w.otRate}/hr</span>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
function renderAttendance() {
  ...
  if (!DB.workers.length) {
    document.getElementById('attendance-list').innerHTML = `<div class="empty">
      <div class="empty-icon">👷</div>
      <div class="empty-title">${t('empty_no_workers')}</div>
      <div class="empty-sub">${t('empty_add_worker_tip')}</div>
    </div>`;
    return;
  }
  ...
  document.getElementById('att-day-total').textContent = dayTotal > 0 ? `${t('att_expense_today')} ₹${dayTotal.toLocaleString('en-IN')}` : '';
  ...
  document.getElementById('attendance-list').innerHTML = DB.workers.map(w => {
    ...
    return `<div class="att-worker-row">
      ...
      <div class="att-btns">
        <button class="att-btn ${status==='present'?'att-present':''}" onclick="markAtt('${date}','${w.id}','present')">✓ ${t('present')}</button>
        <button class="att-btn ${status==='absent'?'att-absent':''}" onclick="markAtt('${date}','${w.id}','absent')">✗ ${t('absent')}</button>
        <button class="att-btn ${status==='half'?'att-half':''}" onclick="markAtt('${date}','${w.id}','half')">½ ${t('half_day')}</button>
        <button class="att-btn ${status==='overtime'?'att-overtime':''}" onclick="markOT('${date}','${w.id}')">⏰ ${t('overtime')}</button>
      </div>
    </div>`;
  }).join('');
}
...
function markOT(date, workerId) {
  ...
  const hrs = prompt(t('prompt_ot_hrs', { name: w.name }), '2');
  ...
  saveDB();
  renderAttendance();
  showToast(t('toast_ot_saved', { name: w.name, hrs: hrs }));
}
...
function addAdvance() {
  ...
  if (!workerId || !amount || !date) { showToast(t('toast_fields_req'), 'error'); return; }
  ...
  renderAdvances();
  renderDashboard();
  showToast(t('toast_advance_given', { name: w.name, amount: amount }));
}

function deleteAdvance(id) {
  if (!confirm(t('confirm_delete_adv'))) return;
...
function renderAdvances() {
  ...
  if (!DB.advances.length) {
    el.innerHTML = `<div class="empty">
      <div class="empty-icon">💸</div>
      <div class="empty-title">${t('empty_no_advance')}</div>
      <div class="empty-sub">${t('empty_advance_tip')}</div>
    </div>`;
    return;
  }
...
function renderHisab() {
  ...
  if (!DB.workers.length) {
    document.getElementById('hisab-table-wrap').innerHTML = `<div class="empty"><div class="empty-icon">👷</div><div class="empty-title">${t('empty_no_workers')}</div></div>`;
    return;
  }
    ...
    <div class="summary-card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;">
        <div>
          ...
          <div class="summary-label">${t('stat_pending_advance')} · ${formatDate(from)} — ${formatDate(to)}</div>
        </div>
        <div style="text-align:right;">
          <div style="font-size:13px;color:var(--text3)">${t('wa_msg_earned').replace('💵 ', '').replace(': *{amt}*', '').replace('Kul Kamaaya', t('wa_msg_earned').split(':')[0].replace('💵 ', ''))}</div>
          <div style="font-size:22px;font-weight:800;color:var(--text)">₹${grandTotal.toLocaleString('en-IN')}</div>
          <div style="font-size:13px;color:var(--red);margin-top:2px;">${t('nav_advance')}: −₹${grandAdvance.toLocaleString('en-IN')}</div>
        </div>
      </div>
    </div>`;
  ...
  document.getElementById('hisab-table-wrap').innerHTML = `
    ...
    <table class="hisab-table">
      <thead>
        <tr>
          <th data-i18n="select_worker">${t('select_worker').replace(' *', '')}</th>
          <th data-i18n="nav_attendance">${t('nav_attendance').replace('✅ ', '')}</th>
          <th data-i18n="stat_monthly_exp">${t('stat_monthly_exp').replace('Is Mahine ', '')}</th>
          <th data-i18n="nav_advance">${t('nav_advance').replace('💸 ', '')}</th>
          <th data-i18n="stat_pending_advance">${t('stat_pending_advance').split(' ')[0]}</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
          ...
          <tr>
            ...
            <td>
              <div style="font-size:13px;color:var(--text2)">
                ${(h.presentDays + h.otDays) > 0 ? `<span style="color:var(--green);font-weight:600;">${h.presentDays + h.otDays} ${t('stat_today').toLowerCase().replace('aaj ka din', 'din')}</span>` : ''}
                ${h.halfDays > 0 ? ` + <span style="color:var(--yellow);font-weight:600;">${h.halfDays} ${t('half_day').toLowerCase()}</span>` : ''}
                ${h.otHours > 0 ? ` <span style="color:var(--blue);font-weight:600;">(${t('overtime')} ${h.otHours} hr)</span>` : ''}
                ${h.totalDays === 0 ? `<span style="color:var(--text3)">${t('empty_no_advance').split(' ')[0]}</span>` : ''}
              </div>
            </td>
            ...
            <td>
              <div style="display:flex;gap:8px;">
                <button class="btn btn-outline btn-sm" onclick="showHisabDetail('${w.id}','${from}','${to}')" data-i18n="btn_details">${t('btn_details')}</button>
                ${w.phone ? `<button class="wa-btn" onclick="sendWA('${w.id}','${from}','${to}')" data-i18n="btn_wa">${t('btn_wa')}</button>` : ''}
              </div>
            </td>
          </tr>`).join('')}
      </tbody>
  ...
  document.getElementById('hisab-detail-content').innerHTML = `
    <div style="display:flex;align-items:center;gap:16px;margin-bottom:24px;">
      ...
      <div>
        ...
        <div style="font-size:14px;color:var(--text3)">${t('skills.' + w.skill)} · ₹${w.rate}/${t('stat_today').toLowerCase().replace('aaj ka din', 'din')}</div>
      </div>
    </div>
    
    <div style="background:var(--surface2);border-radius:12px;padding:16px;margin-bottom:20px;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        <div><div style="font-size:12px;color:var(--text3);margin-bottom:2px;" data-i18n="wa_msg_total">${t('wa_msg_total').split(':')[0].replace('📊 ', '')}</div><div style="font-size:22px;font-weight:800;color:var(--text)">${h.totalDays}</div></div>
        <div><div style="font-size:12px;color:var(--text3);margin-bottom:2px;" data-i18n="wa_msg_earned">${t('wa_msg_earned').split(':')[0].replace('💵 ', '')}</div><div style="font-size:22px;font-weight:800;color:var(--green)">₹${h.earned.toLocaleString('en-IN')}</div></div>
        <div><div style="font-size:12px;color:var(--text3);margin-bottom:2px;" data-i18n="wa_msg_adv_title">${t('wa_msg_adv_title').replace('💸 ', '').replace(':', '')}</div><div style="font-size:22px;font-weight:800;color:var(--red)">₹${h.advance.toLocaleString('en-IN')}</div></div>
        <div><div style="font-size:12px;color:var(--text3);margin-bottom:2px;" data-i18n="stat_pending_advance">${t('stat_pending_advance')}</div><div style="font-size:22px;font-weight:800;color:var(--accent)">₹${h.net.toLocaleString('en-IN')}</div></div>
      </div>
    </div>
    
    ${attDetails.length ? `
    <div style="font-size:13px;font-weight:700;color:var(--text3);margin-bottom:10px;text-transform:uppercase;letter-spacing:0.5px;" data-i18n="nav_attendance">${t('nav_attendance').replace('✅ ', '')} Detail</div>
    <div style="max-height:220px;overflow-y:auto;padding-right:8px;margin-bottom:16px;">
      ${attDetails.map(a => `
        <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);font-size:14px;">
          ...
          <span>${a.status==='present'?'<span style="color:var(--green);font-weight:600;">✓ '+t('present')+'</span>':a.status==='absent'?'<span style="color:var(--red);font-weight:600;">✗ '+t('absent')+'</span>':a.status==='half'?'<span style="color:var(--yellow);font-weight:600;">½ '+t('half_day')+'</span>':'<span style="color:var(--blue);font-weight:600;">✓ '+t('present')+' + ⏰ '+t('overtime')+' '+(a.otH||0)+'hr</span>'}</span>
          <span style="font-weight:700;color:var(--text)">₹${a.amt.toLocaleString('en-IN')}</span>
        </div>`).join('')}
    </div>` : ''}
    
    ${advList.length ? `
    <div style="font-size:13px;font-weight:700;color:var(--text3);margin:16px 0 10px;text-transform:uppercase;letter-spacing:0.5px;" data-i18n="nav_advance">${t('nav_advance').replace('💸 ', '')} List</div>
    ...
    <div style="margin-top:24px;display:flex;gap:12px;flex-direction:column;">
      <button class="btn btn-outline" style="justify-content:center;font-size:15px;font-weight:700;color:var(--text);border-color:var(--text3);" onclick="window.print()" data-i18n="btn_print">${t('btn_print')}</button>
      ${w.phone ? `<button class="wa-btn" style="justify-content:center;padding:12px;" onclick="sendWA('${w.id}','${from}','${to}')" data-i18n="btn_wa_send"><span style="font-size:18px;">📲</span> ${t('btn_wa_send')}</button>` : `<div style="font-size:13px;color:var(--text3);text-align:center;width:100%;background:rgba(255,255,255,0.05);padding:10px;border-radius:10px;" data-i18n="no_phone_tip">${t('no_phone_tip')}</div>`}
    </div>`;
...
function sendWA(wId, from, to) {
  ...
  let msg = t('wa_msg_header', { name: w.name.toUpperCase() }) + '\n';
  msg += t('wa_msg_period', { from: formatDate(from), to: formatDate(to) }) + '\n\n';
  msg += `━━━━━━━━━━━━━━━\n`;
  const totalHaazir = h.presentDays + h.otDays;
  msg += t('wa_msg_days', { days: totalHaazir }) + '\n';
  if (h.halfDays) msg += t('wa_msg_half', { days: h.halfDays }) + '\n';
  if (h.otHours > 0) msg += t('wa_msg_ot', { hrs: h.otHours }) + '\n';
  msg += t('wa_msg_total', { days: h.totalDays }) + '\n\n';
  
  msg += t('wa_msg_rate', { rate: w.rate }) + '\n';
  if (h.otHours > 0 && w.otRate) msg += t('wa_msg_ot_rate', { rate: w.otRate }) + '\n';
  msg += t('wa_msg_earned', { amt: h.earned.toLocaleString('en-IN') }) + '\n';
  if (h.advance > 0) {
    msg += '\n' + t('wa_msg_adv_title') + '\n';
    advList.forEach(a => { msg += `   • ${formatDate(a.date)}: ₹${a.amount}${a.note?' ('+a.note+')':''}\n`; });
    msg += t('wa_msg_adv_total', { amt: h.advance.toLocaleString('en-IN') }) + '\n';
  }
  msg += `\n━━━━━━━━━━━━━━━\n`;
  msg += t('wa_msg_net', { amt: h.net.toLocaleString('en-IN') }) + '\n';
  msg += `━━━━━━━━━━━━━━━\n\n`;
  msg += t('wa_msg_footer');
...
function renderDashboard() {
  ...
  const d = new Date();
  const localeMap = { 'en': 'en-IN', 'hi-hg': 'hi-IN', 'hi-dv': 'hi-IN' };
  const monthStr = d.toLocaleDateString(localeMap[currentLang] || 'hi-IN', { month: 'long', year: 'numeric' });
  ...
  document.getElementById('dash-attendance-list').innerHTML = DB.workers.map(w => {
    ...
    const statusMap = {
      ...
      half:    { label: '½ ' + t('half_day'), cls: 'tag-yellow' },
      overtime:{ label: '⏰ ' + t('overtime'), cls: 'tag-blue' },
      none:    { label: t('lang_hi_hg').split(' ')[0] + ' ' + t('btn_remove').split(' ')[1] + ' ' + t('nav_hisab').split(' ')[1], cls: '' } // Fallback for "Not Marked"
    };
    // Let's refine the "none" label to be more consistent
    statusMap.none.label = currentLang === 'en' ? 'Not Marked' : (currentLang === 'hi-dv' ? 'दर्ज नहीं' : 'Mark Nahi');
    ...
    return `<div class="att-worker-row" style="padding:12px 0;">
      <div class="att-info">
        ...
        <div>
          ...
          <div class="att-rate" style="font-size:12px;">${t('skills.' + w.skill)} · ₹${w.rate}/${t('stat_today').toLowerCase().replace('aaj ka din', 'din')}</div>
        </div>
      </div>
      <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
        ${sm.cls ? `<span class="worker-tag ${sm.cls}" style="font-size:12px;padding:4px 10px;">${sm.label}</span>` : `<span style="font-size:13px;color:var(--text3);font-weight:500;">${statusMap.none.label}</span>`}
        <button class="btn btn-outline btn-sm" onclick="showPage('attendance')" data-i18n="nav_hisab">${t('nav_hisab').replace('📊 ', '')} Mark Karo</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderSettings() {
  // Sync lang card UI with current language
  if (typeof updateLangCards === 'function') updateLangCards();
  // Sync security button states
  const pinStored = localStorage.getItem('hisabkitab_pin');
  const pinBtn = document.getElementById('sec-pin-btn');
  const pinText = document.getElementById('sec-pin-text');
  if (pinBtn && pinText) {
    if (pinStored) {
      pinText.textContent = t('pin_change_title') || '4-Digit PIN Change Karein';
      pinBtn.textContent = t('btn_change') || 'Change';
    } else {
      pinText.textContent = t('pin_setup_title');
      pinBtn.textContent = t('btn_setup');
    }
  }
  const bioBtn = document.getElementById('sec-bio-btn');
  if (bioBtn) {
    const bioEnabled = localStorage.getItem('hisabkitab_bio') === 'true';
    bioBtn.textContent = bioEnabled ? (t('btn_disable') || 'Disable') : (t('btn_enable'));
  }
}
...
function handleClearData() {
  if (confirm(t('confirm_clear_all'))) {
    clearAllData();
    showToast(t('toast_clear_success'), "error");
    renderDashboard();
    showPage('dashboard');
  }
}
...
function addPurchase() {
  ...
  if (!materialId || !qty || !rate || !date) { showToast(t('toast_purchase_details_req'), 'error'); return; }
  ...
  renderMaterials();
  showToast(t('toast_purchase_saved'));
}

function deletePurchase(pId) {
  if (!confirm(t('confirm_delete_purchase'))) return;
  DB.purchases = DB.purchases.filter(p => p.id !== pId);
  saveDB();
  renderMaterials();
  showToast(t('toast_purchase_deleted'), 'error');
}
...
function renderMaterials() {
  ...
  if (!DB.purchases || !DB.purchases.length) {
    statsEl.innerHTML = '';
    listEl.innerHTML = `<div class="empty"><div class="empty-icon">🧱</div><div class="empty-title">${t('empty_no_materials')}</div><div class="empty-sub">${t('empty_materials_tip')}</div></div>`;
    return;
  }
  ...
  statsEl.innerHTML = `
    <div class="stat-card">
      <div class="stat-label" data-i18n="stat_total_mat_exp">${t('stat_total_mat_exp')}</div>
      <div class="stat-val accent">₹${totalCost.toLocaleString('en-IN')}</div>
      <div class="stat-sub" data-i18n="stat_total_mat_sub">${t('stat_total_mat_sub')}</div>
    </div>
    <div class="stat-card">
      <div class="stat-label" data-i18n="stat_monthly_mat">${t('stat_monthly_mat')}</div>
      <div class="stat-val green">₹${thisMonthCost.toLocaleString('en-IN')}</div>
      <div class="stat-sub" data-i18n="stat_monthly_mat_sub">${t('stat_monthly_mat_sub')}</div>
    </div>
  `;
}
...
function updateMatDefaultRate(id, val) {
  ...
  if (mat) {
    ...
    saveDB();
    showToast(t('toast_rate_updated'));
  }
}
...
function deleteMaterial(id) {
  ...
  saveDB();
  renderManageMaterials();
  showToast(t('toast_material_deleted'), 'error');
}
...
function addNewMaterial() {
  ...
  if (!name || !unit || !rate) {
    showToast(t('toast_fields_req'), 'error');
    return;
  }
  ...
  renderManageMaterials();
  showToast(t('toast_material_added'));
}
...
function processPIN() {
  if (setupMode) {
    if (!setupPINValue) {
      ...
      currentPIN = "";
      updatePINDots();
      document.getElementById('lock-title').textContent = t('lock_title_confirm');
    } else {
      if (currentPIN === setupPINValue) {
        SEC.pin = currentPIN;
        saveSec();
        showToast(t('toast_pin_set'), "success");
        hideLockScreen();
      } else {
        showToast(t('toast_pin_mismatch'), "error");
        currentPIN = "";
        setupPINValue = null;
        updatePINDots();
        document.getElementById('lock-title').textContent = t('lock_title_setup');
      }
    }
  } else {
    if (currentPIN === SEC.pin) {
      ...
      showToast(t('toast_wrong_pin'), "error");
      currentPIN = "";
      updatePINDots();
    }
  }
...
function setupPIN() {
  if (SEC.pin) {
    if (confirm(t('confirm_remove_lock'))) {
      SEC.pin = null;
      SEC.bio = false;
      SEC.credentialId = null;
      saveSec();
      renderSettingsScreen();
      showToast(t('toast_lock_removed'), "error");
    }
    return;
  }
  setupMode = true;
...
function renderSettingsScreen() {
  ...
  if (SEC.pin) {
    pinText.textContent = t('pin_active');
    pinBtn.textContent = t('btn_remove');
    pinBtn.className = "btn btn-red";
  } else {
    pinText.textContent = t('pin_inactive');
    pinBtn.textContent = t('btn_setup');
    pinBtn.className = "btn btn-outline";
  }

  if (SEC.bio) {
    bioBtn.textContent = t('btn_disable');
    bioBtn.className = "btn btn-red";
  } else {
    bioBtn.textContent = t('btn_enable');
    bioBtn.className = "btn btn-outline";
  }
}

function toggleBiometric() {
  if (!SEC.pin) return showToast(t('toast_pin_first'), "error");
  ...
  if (SEC.bio) {
    ...
    saveSec();
    renderSettingsScreen();
    return showToast(t('toast_bio_disabled'), "error");
  }

  if (!window.PublicKeyCredential) {
    return showToast(t('toast_bio_not_supported'), "error");
  }
  ...
  navigator.credentials.create({
    ...
    saveSec();
    renderSettingsScreen();
    showToast(t('toast_bio_success'), "success");
  }).catch(e => {
    console.error(e);
    showToast(t('toast_bio_fail', { msg: e.message }), "error");
  });
}
...
function tryBiometric() {
  ...
  navigator.credentials.get({
    ...
    showToast(t('toast_bio_unlocked'), "success");
    hideLockScreen();
  }).catch(e => {
    console.error(e);
...
function initSecurity() {
  ...
  if (SEC.pin) {
    document.getElementById('app-lock-screen').classList.remove('hidden');
    document.getElementById('lock-title').textContent = t('lock_title_pin');
    if (SEC.bio) {
